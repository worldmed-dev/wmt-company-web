...
function MegaMenu({ menu, onClose }: { menu: Menu; onClose: () => void }) {
  ...
  return (
    <div className="w-full backdrop-blur-3xl bg-white/40 shadow-2xl border-y border-black/10 flex" style={{ animation: 'megaMenuIn 0.3s cubic-bezier(0.16, 1, 0.3, 1) forwards' }}>
      ...
      <div className="flex-1 p-8 flex flex-col gap-y-1">
        {menu.items.map((item) => (
          <Link key={item.title} href={(item.href ?? '#') as never} onMouseEnter={() => setActiveItem(item)} onClick={onClose}
            ...
            <span className={`text-[16px] font-semibold transition-colors ${activeItem.title === item.title ? 'text-[#112246]' : 'text-[#112246]/60'}`}>{item.title}</span>
        ...
        {activeItem.sub ? (
          activeItem.sub.map((sub) => (
            <Link key={sub.title} href={(sub.href ?? '#') as never} onClick={onClose} className="group/sub flex items-center justify-between py-2.5 px-4 rounded-xl hover:bg-[#112246]/10 transition-colors">
              <span className="text-[#112246]/60 text-[16px] font-semibold group-hover/sub:text-[#112246] transition-colors">{sub.title}</span>
...
export default function Navbar({ categories }: { categories: CategoryWithSubs[] }) {
  ...
  useEffect(() => {
    const handleClickOutside = (e: MouseEvent) => {
      ...
      if (nav && !nav.contains(e.target as Node)) setOpenMenu(null);
    };
    document.addEventListener('mousedown', handleClickOutside);
    return () => document.removeEventListener('mousedown', handleClickOutside);
  }, []);

  const switchLocale = (next: string) => router.replace(pathname, { locale: next as 'en' | 'th' });
  ...
  return (
    <SiteNavbar
        ...
        <div className="relative flex items-center gap-12">
          {menus.map((menu) => (
            <button key={menu.key}
              ...
              className="flex items-center gap-1 text-[16px] font-bold text-[#112246] uppercase cursor-pointer">
              {menu.label}
              <ChevronDownIcon className={`w-4 h-4 transition-transform duration-200 ${openMenu === menu.key ? 'rotate-180' : ''}`} />
            </button>
          ))}
          <a href={careerSiteUrl}
            className="relative text-[16px] font-bold text-[#112246]/70 hover:text-[#112246] uppercase transition-colors after:absolute after:bottom-0 after:left-0 after:h-[2px] after:w-0 after:bg-[#112246] after:transition-all after:duration-300 hover:after:w-full">
            {t('nav.career')}
          </a>
        </div>
      }
      belowNavSlot={
        openMenu ? (
          <MegaMenu menu={menus.find(m => m.key === openMenu)!} onClose={() => setOpenMenu(null)} />
        ) : null
      }
      rightSlot={
        <div className="flex items-center gap-6" onMouseEnter={() => setOpenMenu(null)}>
          ...
          <div className="relative group">
            ...
            <div className="absolute top-full right-0 mt-2 w-36 backdrop-blur-md bg-white/80 border border-[#112246]/20 overflow-hidden opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200">
              <button onClick={() => switchLocale('en')} className={`w-full px-4 py-2.5 text-left text-sm font-bold transition-colors hover:bg-[#112246]/10 ${locale === 'en' ? 'text-[#112246]' : 'text-[#112246]/60'}`}>{t('nav.lang_en')}</button>
              <button onClick={() => switchLocale('th')} className={`w-full px-4 py-2.5 text-left text-sm font-bold transition-colors hover:bg-[#112246]/10 ${locale === 'th' ? 'text-[#112246]' : 'text-[#112246]/60'}`}>{t('nav.lang_th')}</button>
            </div>
          </div>
        </div>
      }
      mobileNavSlot={
        <div className="flex flex-col gap-1">
          ...
          <a href={careerSiteUrl} className="py-3 text-sm font-bold text-white/70 uppercase border-b border-white/10">{t('nav.career')}</a>
